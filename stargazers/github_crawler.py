...
# I know, it's a copy-paste of the previous function :)
def get_starred_repos(username: str, access_token:str) -> typing.List[typing.Tuple]:
...
def fill_db(owner, repo, access_token):
    ...
    # Use a map to keep track of added objects to avoid redundant adding
    user_map = dict()
    repo_map = dict()

    def _get_or_create_user_model(user_name):
        model_user = user_map.get(user_name)
        if model_user is None:
            user_map[user_name] = model_user = User(user_name=user_name)
        return model_user

    def _get_or_create_repo_model(repo_name):
        model_repo = repo_map.get(repo_name)
        if model_repo is None:
            repo_map[repo_name] = model_repo = Repository(name=repo_name)
        return model_repo
    ...
    # My token has limits :)
    for gazer in star_gazers[:10]:
        model_gazer = _get_or_create_user_model(gazer)
        ...
        for _repo, _owner in starred_repos[:10]:
            model_owner = _get_or_create_user_model(_owner)
            model_repo = _get_or_create_repo_model(_repo)

            model_repo.owner = model_owner
            model_repo.starred_by.append(model_gazer)

    logger.info(f"adding {len(user_map)} users")
    logger.info(f"adding {len(repo_map)} repo")
